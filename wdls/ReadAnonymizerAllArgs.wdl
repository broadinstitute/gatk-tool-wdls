...
# Run ReadAnonymizer **EXPERIMENTAL** ReadAnonymizer (WDL auto generated from GATK Version 4.2.3.0-SNAPSHOT)
#
...
#    ref_base_quality                                   Quality for bases that are set to the reference base.       
#    sites_only_vcf_output                              If true, dont emit genotype fields when writing vcf file output.
#    use_simple_cigar                                   If true, will produce a simplified cigar string (without `=` and `X`).
#
...
workflow ReadAnonymizer {
  ...
  input {
    ...
    Int? ref_base_quality
    Boolean? sites_only_vcf_output
    Boolean? use_simple_cigar
  ...
  call ReadAnonymizer {
    ...
    input:
        ...
        use_simple_cigar                                   = use_simple_cigar,
  ...
  output {
    # Workflow Outputs                                  
    File ReadAnonymizeroutput_arg = ReadAnonymizer.ReadAnonymizer_output_arg
    File? ReadAnonymizeroutputIndex = ReadAnonymizer.ReadAnonymizer_outputIndex
  }
  ...
  parameter_meta {
    ...
    output_arg: { description: "Output bam file." }
    ...
    ref_base_quality: { description: "Quality for bases that are set to the reference base." }
    sites_only_vcf_output: { description: "If true, dont emit genotype fields when writing vcf file output." }
    use_simple_cigar: { description: "If true, will produce a simplified cigar string (without `=` and `X`)." }
...
task ReadAnonymizer {
  ...
  command <<<
    ...
    ~{true='--ref-base-quality ' false='' defined(ref_base_quality)}~{sep=' --ref-base-quality ' ref_base_quality} \
    ~{true='--sites-only-vcf-output ' false='' defined(sites_only_vcf_output)}~{sep=' --sites-only-vcf-output ' sites_only_vcf_output} \
    ~{true='--use-simple-cigar ' false='' defined(use_simple_cigar)}~{sep=' --use-simple-cigar ' use_simple_cigar} \
  ...
  output {
    # Task Outputs                                      
    File ReadAnonymizer_output_arg = output_arg
    File? ReadAnonymizer_outputIndex = outputIndex
  }
