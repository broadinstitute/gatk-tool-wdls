...
# Run ClipReads (WDL auto generated from GATK Version 4.2.4.1-SNAPSHOT)
#
...
#    cycles_to_trim                                     String indicating machine cycles to clip from the reads     
...
workflow ClipReads {
  ...
  output {
    # Workflow Outputs                                  
    File ClipReadsoutput_arg = ClipReads.ClipReads_output_arg
    File? ClipReadsoutputIndex = ClipReads.ClipReads_outputIndex
    File? ClipReadsoutput_statistics = ClipReads.ClipReads_output_statistics
  }
  ...
  parameter_meta {
    ...
    output_arg: { description: "BAM output file" }
    ...
    cycles_to_trim: { description: "String indicating machine cycles to clip from the reads" }
...
task ClipReads {
  ...
  command <<<
    ...
    ~{true='--cycles-to-trim ' false='' defined(cycles_to_trim)}~{sep=' --cycles-to-trim ' cycles_to_trim} \
  ...
  output {
    # Task Outputs                                      
    File ClipReads_output_arg = output_arg
    File? ClipReads_outputIndex = outputIndex
    File? ClipReads_output_statistics = output_statistics
  }
